...
class table_loader():
    def __init__(self):
        ...
        # end_t = return_time(1)
        # time_contrainedMetaFD = self.metaDataDF[
        #     (self.metaDataDF['release_date'] > start_t) & (self.metaDataDF['release_date'] < end_t)]
        # genres = "All"
        # if genres == 'All':
        #     pass
        # else:
        ...
        # scoreDF = pd.merge(self.movie_actorDF, time_contrainedMetaFD)[['actor_id', 'final_score']].groupby(
        #     'actor_id').sum()
        # scoreDF = scoreDF / scoreDF.sort_values('final_score', ascending=False).iloc[:max_num].sum()
        # time_contrainedMetaFD = (time_contrainedMetaFD.assign(
        #     relative_position=(time_contrainedMetaFD.release_date - start_d) / (end_d - start_d)))
        # self.time_contrainedMetaFD = time_contrainedMetaFD

    def return_filtered(self, start_float, end_float, max_num=50, genres={}, num_columns=50):
        genres_list=[]
        for genre in genres:
            if genres[genre]==1:
        ...
        if genres_list == []:
            ...
            table = self.movie_to_genreDF[self.movie_to_genreDF.name.isin(genres_list)][['id', 'name']].groupby('id').count()

            table = table[table.name == len(genres_list)]
            print(table)
            table['id'] = table.index
        ...
        final_table = pd.merge(self.movie_actorDF, time_contrainedMetaFD)[['actor_id', 'relative_position']].groupby(
            'actor_id').mean().join(self.actorDF).join(scoreDF).sort_values('final_score', ascending=False).iloc[
                      :max_num]
        final_table = final_table.assign(actor_id=final_table.index)
        columnList =  time_contrainedMetaFD[time_contrainedMetaFD.revenue != 0].groupby(
            pd.cut(time_contrainedMetaFD[time_contrainedMetaFD.revenue != 0]["relative_position"],np.arange(0, 1.0 + 1 / num_columns, 1 / num_columns)))['revenue'].mean().reset_index()['revenue'].fillna(0).tolist()
        ...
        return json.dumps({"actorsDatajson":final_table.to_dict('records'),"columnList":columnList})
    ...
    #                    np.arange(0, 1.0 + 1 / num_columns, 1 / num_columns)))['revenue'].mean().reset_index()[
    #             'revenue'].fillna(0).tolist()
    ...
    #         return self.time_contrainedMetaFD[self.time_contrainedMetaFD.revenue != 0].groupby(
    #             pd.cut(self.time_contrainedMetaFD[self.time_contrainedMetaFD.revenue != 0]["relative_position"],
    #                    np.arange(0, 1.0 + 1 / num_columns, 1 / num_columns)))['revenue'].sum().reset_index()[
    #             'revenue'].fillna(0).tolist()
            #     pd.DataFrame(self.time_contrainedMetaFD[self.time_contrainedMetaFD.revenue!=0].groupby(
            # pd.cut(self.time_contrainedMetaFD[self.time_contrainedMetaFD.revenue!=0]["relative_position"],
    ...
    def return_actor_network(self, actor_id):
        pd.merge(pd.merge(self.movie_actorDF[self.movie_actorDF.actor_id == actor_id], self.metaDataDF),
                 self.movie_to_genreDF)
        DF = pd.merge(
            pd.merge(pd.merge(self.movie_actorDF[self.movie_actorDF.actor_id == actor_id], self.metaDataDF),
                     self.movie_to_genreDF)[
        ...
        actor_dict_list = [{'name': actor_dictionary[actor_id]['name'], 'gender': actor_dictionary[actor_id]['gender'],
                            ...
                            'actor_id': actor_dictionary[actor_id]['actor_id']}]
        links_list = []
        skip = 0
        for index, (index_row, row) in enumerate(DF4.iterrows()):

            links_list.append(
                {'source': actor_dictionary[actor_id]['name'], "target": actor_dictionary[row.actor_id]['name'],
                 'value': row.genre_id})
            ...
            actor_dict_list.append(
                {'name': actor_dictionary[row.actor_id]['name'], 'gender': actor_dictionary[row.actor_id]['gender'],
                 ...
                 'actor_id': actor_dictionary[row.actor_id]['actor_id']})
