...
function scaleRadialGraphInitialize(inputdata) {
    ...
    g = d3.select("#Chartsvg")
        .append("g")
        .attr("transform", "translate(" + width / 2 + "," + height / 2 + ")")
        ;
    //g = svg.select("svg").select("g");
    var x = d3.scaleBand()
        .range([0, 2 * Math.PI])
        .align(0);
...
function ScaleRadialGraphUpdate(inputdata) {
    ...
    var arc = d3.arc()
        .innerRadius(innerRadius)
        .outerRadius(outerRadius);
    svg.select("defs").selectAll("radialGradient").data(pie(data))
        .attr("r", "40%")
        .attr("id", function(d, i) { return "grad" + i; })
        .append("stop").attr("offset", "65%").style("stop-color", function(d, i) {return d3.rgb("#281437").darker(1); })
        .append("stop").attr("offset", "100%").style("stop-color", function(d, i) { return color(d.value); })
        .attr("stop-opacity",0.8);
